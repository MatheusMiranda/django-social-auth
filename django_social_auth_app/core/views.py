...
def home(request):
    ...
    if tags_filter_param:
        filtered_tags = Tag.objects.\
            filter(reduce(operator.or_,
                          (Q(title__contains=search.strip()) for search in
                           tags_filter_param.split(','))))
        repositories_list = Repository.objects.filter(owner_id=user.id).\
            filter(tags__in=filtered_tags).distinct()
        on_filter = True
    else:
