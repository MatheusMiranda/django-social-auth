...
def instantiate_repository(repository, user):
    ...
    obj, created = Repository.objects.update_or_create(
        name=name, owner=user, defaults={"description": description}
    )
...
def edit_repository_tags(request, repository_id):
    ...
    return render(
        request,
        "repository/edit/edit_tags.html",
        {"repository": repository, "form": form, "tags_table": tags_table},
    )
...
def remove_repository(request, repository_id):
    Repository.objects.get(id=repository_id).delete()
...
def get_repositories(request):
    ...
    response = requests.get(
        "https://api.github.com/users/{}/repos".format(github_user)
    )
